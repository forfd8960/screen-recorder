...
**Tests**: Tests are REQUIRED by constitution. Include unit/integration/contract tests as applicable for each user story.
...
- [ ] T004 Configure required verification commands (format, lint, test, and security checks when applicable)
...
- [ ] TXXX Run required verification suite (`cargo fmt`, `cargo clippy --all-targets --all-features`, `cargo test`)
- [ ] TXXX Run dependency/security checks (`cargo audit`, `cargo deny`) when dependencies or risk profile changed
