...
- [X] T058 [P] [US6] Write unit test in `src/capture/engine.rs`: `build_stream_config(settings)` with `VideoQuality::Low` sets `minimumFrameInterval` for 24 FPS and `AVVideoAverageBitRateKey = 2_000_000`; same for `Medium` / `High`; `Resolution::P1080` sets `height = 1080`
- [X] T059 [P] [US6] Write unit test `RecordingSettings` round-trip with all three `VideoQuality` and all three `Resolution` variants via serde JSON

### Implementation for User Story 6

- [X] T060 [US6] Implement `build_stream_config(settings: &RecordingSettings) -> SCStreamConfiguration` in `src/capture/engine.rs`: width/height from `Resolution`; `minimumFrameInterval` from `frame_rate`; NV12 pixel format; audio enabled
- [X] T061 [US6] Implement `VideoQuality → bitrate` mapping in `src/encode/pipeline.rs`: `High = 8_000_000`, `Medium = 4_000_000`, `Low = 2_000_000` bps; passed to `AVVideoAverageBitRateKey` in compression properties dict
